...
import {getAllArticles,editArticle,deleteArticle,addArticle} from '../../service/article-service'
...
export default function ArticleEditor(req,res) {
  ...
  return (
    ...
    <tr>
    <div><b>ID :</b> {type._id}</div>
    <div><b>TITLE:</b>  {type.title}</div>
    <div><b>Description:</b> {type.description}</div>
    <div><b>Markdown :</b> {type.markdown}</div>
    </tr>
     </div>)})}
    </div>
    </>
